...
import (
	...
	"github.com/OtchereDev/ris-ms-dicom-server/internal/io-dicom/network/pdutype"
)
...
type PDUService interface {
	...
	readPDU() error
	SetConnectionInfo(conn net.Conn)
}
...
// Add helper function for extracting host/port
func extractHostPort(conn net.Conn) (string, int) {
	if conn == nil {
		return "", 0
	}
	...
	// Handle IPv6 addresses with brackets: [::1]:12345
	if strings.HasPrefix(remoteAddr, "[") {
		if idx := strings.LastIndex(remoteAddr, "]:"); idx != -1 {
			host := remoteAddr[1:idx]
			if portStr := remoteAddr[idx+2:]; portStr != "" {
	...
	// Handle IPv4 addresses: 192.168.1.100:11112
...
func (pdu *pduService) SetConnectionInfo(conn net.Conn) {
	pdu.conn = conn

	if conn != nil {
		host, port := extractHostPort(conn)
		if aarq, ok := pdu.AssocRQ.(*aassociationRQ); ok {
			aarq.SetCallingHost(host)
			aarq.SetCallingPort(port)
		}
	}
}
