...
type SCP interface {
	...
	OnAssociationClose(f func(request network.AAssociationRQ))
}
...
type scp struct {
	...
	onAssociationClose   func(request network.AAssociationRQ)
}
...
func (s *scp) OnAssociationClose(f func(request network.AAssociationRQ)) {
	s.onAssociationClose = f
}
...
func (s *scp) handleConnection(conn net.Conn) {
	...
	defer func() {
		...
		if s.onAssociationClose != nil {
			s.onAssociationClose(pdu.GetAAssociationRQ())
		}
	}()
